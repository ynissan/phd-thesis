\documentclass[12pt,a4paper]{book}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{enumitem}
\usepackage[left=2cm,right=2cm,top=2cm,bottom=2cm]{geometry}
\usepackage{hyperref} 
\usepackage{graphicx}
\usepackage[xindy,toc,acronym]{glossaries}
\usepackage{ifthen}
%\usepackage{utphys}
\hypersetup{pdftitle={Search for compressed mass Higgsino production with soft
lepton tracks with the CMS experiment in pp collision data at 13 TeV}, pdfauthor={Yuval Nissan}}
\usepackage{ptdr-definitions}
%\usepackage{heppennames2}
\usepackage[pazoGreek]{heppennames2}
%\usepackage[italic,italicGreek]{heppennames2}
\usepackage[nomargin,inline,index]{fixme}
\usepackage[numbers,sort&compress]{natbib}

\fxsetup{inline=true,status=draft}
% this is to make table captions on top
\usepackage{floatrow}
\floatsetup[table]{capposition=top}

\makeglossaries

\input{commands}
\input{glossary}
\input{acronyms}

\begin{document}

% this is for book class what comes as the front
\frontmatter

\thispagestyle{empty}

\title{
        \LARGE \textbf{Search for compressed mass Higgsino production with soft
lepton tracks with the CMS experiment in proton-proton collision data at $\mathbf{\sqrt{s}=13}$ TeV} \\[1.2cm]
        %~ {\normalsize\textsc{
        {\large\textsc{
		Von der Fakult\"at für Physik
		der Universit\"at Hamburg zur Erlangung des akademischen Grades eines
		Doktors der Naturwissenschaften genehmigte Dissertation
		\\[1.2cm]
		vorgelegt von
		\\[1.2cm]
		Yuval Nissan\\
		im August 2022
        }}
\vfill
\begin{center}
        \includegraphics[width=125mm]{uhh-logo.png}
\end{center}        
        
}
\author{}
\date{}
\maketitle


\pagenumbering{gobble}
\thispagestyle{empty}

\pdfbookmark[0]{Titlepage}{title}

\thispagestyle{empty}
\null\vfill


\pagebreak
%\pagenumbering{roman}
%\setcounter{page}{1}
\include{abstract}
\tableofcontents

% this is for book class what comes as the main book 
\mainmatter
%\pagenumbering{arabic}
%\setcounter{page}{1}


\include{introduction}
\include{standard-model}
\include{susy}
\include{bdt}
\include{detector}
\include{reconstruction-and-identification}
\include{search}
\include{non-isolated-bg}
\include{summary}
\include{latex-stuff}

\listoffixmes
%\part{Feynman Diagrams and Quantum Electrodynamics}
%\part{Renormalization}
%\part{Definitions and Mathematical tools}
%\printglossary[type=\acronymtype]
\printglossaries


% this is for book class what comes as the back of book 
\backmatter
\newpage{}
\bibliographystyle{utphys}
\bibliography{references} 
\listoffigures
\listoftables

\end{document}
